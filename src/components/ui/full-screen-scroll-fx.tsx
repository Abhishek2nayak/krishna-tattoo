...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      // init subtitles
      subRefs.current.forEach((el, sIdx) => {
        if (!el) return;
        gsap.set(el, { opacity: sIdx === index ? 1 : 0, y: sIdx === index ? 0 : 16 });
      });
    ...
    const changeSection = (to: number) => {
      ...
      // Animate subtitles
      const outSub = subRefs.current[from];
      const inSub = subRefs.current[to];
      if (outSub) gsap.to(outSub, { opacity: 0, y: down ? -12 : 12, duration: D * 0.5, ease: "power2.out" });
      if (inSub) {
        gsap.set(inSub, { opacity: 0, y: down ? 16 : -16 });
        gsap.to(inSub, { opacity: 1, y: 0, duration: D, delay: D * 0.3, ease: "power3.out" });
      }
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-text" as any]: colors.text ?? "rgba(240,240,240,0.92)",
      ["--fx-overlay" as any]: colors.overlay ?? "rgba(0,0,0,0.55)",
      ["--fx-page-bg" as any]: colors.pageBg ?? "#060606",
      ["--fx-stage-bg" as any]: colors.stageBg ?? "#060606",
      ...
      >
          ...
          <div className="fx-fixed-section" ref={fixedSectionRef}>
            <div className="fx-fixed" ref={fixedRef}>
              ...
              <div className="fx-bgs" aria-hidden="true">
                {sections.map((s, i) => (
                  <div className={`fx-bg ${i === index ? "fx-bg-active" : ""}`} key={s.id ?? i}>
                    {s.renderBackground ? (
                    ...
                    {sections.map((s, sIdx) => {
                      ...
                      return (
                        <div key={`C-${s.id ?? sIdx}`} className={`fx-featured ${sIdx === index ? "active" : ""}`}>
                          ...
                          {(s.subtitle || s.ctaHref) && (
                            <div
                              className="fx-featured-meta"
                              ref={(el) => { if (el) subRefs.current[sIdx] = el }}
                            >
                              {s.subtitle && (
                                <p className="fx-featured-sub">{s.subtitle}</p>
                              )}
                              {s.ctaHref && (
                                <Link
                                  href={s.ctaHref}
                                  className="fx-featured-cta"
                                  onClick={(e) => e.stopPropagation()}
                                >
                                  {s.ctaLabel || "View in Gallery"}
        ...
        <style jsx>{`
          ...
          .fx-debug {
            ...
            background: rgba(232,255,0,0.9); color: #fff; padding: 6px 8px; font: 12px/1 monospace; border-radius: 4px;
          }
          ...
          .fx-bg {
            ...
            opacity: 0;
            transition: opacity ${durations.change ?? 0.7}s ease;
          }
          .fx-bg.fx-bg-active { opacity: 1; }
          .fx-bg-img {
            position: absolute; inset: -10% 0 -10% 0;
            width: 100%; height: 120%; object-fit: cover;
            filter: brightness(0.45) contrast(1.2) grayscale(0.1);
          ...
          .fx-left-item.active, .fx-right-item.active { opacity: 1; color: #E8FF00; }
          ...
          .fx-right-item.active::after {
            ...
            width: 5px; height: 5px; background: #E8FF00; border-radius: 50%;
          ...
          .fx-featured { position: absolute; opacity: 0; visibility: hidden; display: flex; flex-direction: column; align-items: center; }
          ...
          .fx-featured-cta {
            ...
            background: #E8FF00; color: #060606;
            text-decoration: none;
            transition: background 0.3s ease, color 0.3s ease, transform 0.3s ease;
            border: 1px solid #E8FF00;
            margin-top: 0.25rem;
          }
          .fx-featured-cta:hover {
          ...
          .fx-progress { width: 180px; height: 1px; margin: 1rem auto 0; background: rgba(240,240,240,0.15); position: relative; }
          .fx-progress-fill { position: absolute; inset: 0 auto 0 0; width: 0%; background: #E8FF00; height: 100%; transition: width 0.3s ease; }
          .fx-progress-numbers { position: absolute; inset: auto 0 calc(100% + 6px) 0; display: flex; justify-content: space-between; font-size: 0.65rem; color: rgba(240,240,240,0.4); letter-spacing: 0.15em; }
